...
import { BrowserRouter, Routes, Route, useOutletContext, useParams } from 'react-router-dom'
...
const routes = <BrowserRouter>
  <Routes>
    ...
    <Route path="/" element={appWithRedux}>
      ...
      <Route path="/react-router/" element={<ReactRouterRoute />} >
        <Route index element={<ReactRouterRoute />} />
        <Route path=":name" element={<ReactRouterRoute />} />
      </Route>
    </Route>
  </Routes>
</BrowserRouter >;
...
function ReactRouterRoute() {
  ...
  return <div>react router is here and welcomes you {params.name}</div>;
}
